...
invCont.buildEditView = async function (req, res, next) {
    ...
    let nav = await utilities.getNav()
    const itemData = await invModel.getVehicleByVehicleId(inv_id)
...
invCont.updateInventory = async function (req, res) {
    ...
    if (updateResult) {
        req.flash(
            "notice",
            `Congratulations, you updated ${inv_make} ${inv_model} to the database.`
        )
        res.redirect("/inv/")
    } else {
...
/* ***************************
 *  Builds the delete confirmation view
 * ************************** */
...
invCont.deleteInventory = async function (req, res) {
    ...
    if (deleteResult) {
        req.flash(
            "notice",
            `Congratulations, you deleted ${inv_make} ${inv_model} from the database.`
        )
        res.redirect("/inv/")
    } else {
        const itemName = `${inv_make} ${inv_model}`
        req.flash("notice", "Sorry, the delete failed.")
        res.status(501).render("inventory/delete-confrim", {
            title: "Edit " + itemName,
            nav,
            errors: null,
            inv_id,
